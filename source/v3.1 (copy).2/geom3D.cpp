...
vector3* intersectSphere(TRay ray, double radius, vector3 res[2])
{
	...
	long double c = pow(ray.start.x,2)+pow(ray.start.y-b,2)+pow(ray.start.z,2)-radius*radius;	
	...
	if(App::isStatMode)
	{
		...
		fprintf(FST, "Radius: %le; Ray: [%le;%le;%le]; b=%Le;c=%Le; D=%Le\n",radius, ray.start.x, ray.start.y, ray.start.z, b, c, D);
