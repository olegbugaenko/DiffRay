...
char** CReader::read_commands(FILE *MF)
{
	...
	if(!feof(MF))
	{
		...
		while(command_part != NULL)
		{
			...
			result[i] = new char[5];
			strncpy(result[i],command_part,5);
			i++;
...
double** CReader::read_file_array(FILE *MF, int maxsize, int nskiprows, int nskipcols, int (*oncalcrows)(int), int (*oncalccols)(int), int (*onelementset)(int,int,double))
{
	double** result;
	if(!MF || MF == NULL)
	{
