...
int CIntegration::InitIntegration(int mode,int nPrecision)
{
	...
	{
		CIntegration::nPhi   = nPrecision*2;
		CIntegration::nTheta = nPrecision;
		//printf("PREC: %le/%d\n",M_PI,nPrecision);
		//printf("nPhi: %d\n",CIntegration::nPhi);
	}
	else
...
int CIntegration::doCalc()
{
	...
	else
	{
		...
		double phi = - (App::phi_width-dphi)/2;
		double theta = - (App::theta_width-dtheta)/2;
		
		double phi_max = (App::phi_width-dphi)/2;
		double theta_max = (App::theta_width-dtheta)/2;
		...
		while(phi<phi_max+dphi/5.0)
		{
			...
			theta = - (App::theta_width-dtheta)/2;
